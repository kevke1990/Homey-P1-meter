...
class P1DongleDevice extends Homey.Device {
  ...
  processTelegram(telegram) {
    try {
      ...
      if (importKw !== null) {
        this.setCapabilityValue('measure_power', Math.round(importKw * 1000))
          .catch(err => this.error('measure_power:', err));
      }
      if (exportKw !== null) {
        this.setCapabilityValue('measure_power.returned', Math.round(exportKw * 1000))
      ...
      if (t1In !== null && t2In !== null) {
        this.setCapabilityValue('meter_power', t1In + t2In)
          .catch(err => this.error('meter_power:', err));
      }
      if (t1Out !== null && t2Out !== null) {
        this.setCapabilityValue('meter_power.returned', t1Out + t2Out)
