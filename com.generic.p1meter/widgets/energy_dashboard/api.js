...
async function getData({ homey, query }) {
  ...
  const todayImport = sum(bucketByDay(import7Delta, 7).slice(-1));
  const todayExport = sum(bucketByDay(export7Delta, 7).slice(-1));
  const todayGas = sum(bucketByDay(gas7Delta, 7).slice(-1));
